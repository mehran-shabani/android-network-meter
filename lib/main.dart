...
import 'models.dart';
import 'native_net_service.dart';
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            _SpeedCard(down: _down, up: _up, activeSim: _activeSim),
            ...
            if (report != null) ...[
              ...
              const SizedBox(height: 10),
              _AppsList(apps: report.apps, selected: _selectedApp, onTap: _selectApp),
            ],
          ],
        ),
      ),
...
class _SpeedCard extends StatelessWidget {
  const _SpeedCard({required this.down, required this.up, required this.activeSim});
  final double down;
  final double up;
  ...
  Widget build(BuildContext context) => Card(
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              Expanded(child: _Metric('دانلود', speed(down), Icons.download_rounded)),
              const SizedBox(width: 8),
              Expanded(child: _Metric('آپلود', speed(up), Icons.upload_rounded)),
            ]),
            const SizedBox(height: 10),
  ...
  Widget build(BuildContext context) => Card(
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              Expanded(child: _Metric('دانلود', bytes(report.totalRxBytes), Icons.download)),
              const SizedBox(width: 8),
              Expanded(child: _Metric('آپلود', bytes(report.totalTxBytes), Icons.upload)),
              const SizedBox(width: 8),
              Expanded(child: _Metric('کل', bytes(report.totalBytes), Icons.data_usage)),
            ]),
            const SizedBox(height: 8),
...
class _AppsList extends StatelessWidget {
  const _AppsList({required this.apps, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) => Card(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (apps.isEmpty) const Padding(padding: EdgeInsets.all(16), child: Text('برنامه‌ای پیدا نشد.')),
          ...apps.take(100).map((app) => ListTile(
                ...
                trailing: Text(bytes(app.totalBytes)),
                onTap: () => onTap(app),
              )),
        ]),
      );
